...
func getConfigChartValues(infraStatus *apisazure.InfrastructureStatus, cp *extensionsv1alpha1.ControlPlane, cluster *extensionscontroller.Cluster, ca *internal.ClientAuth) (map[string]interface{}, error) {
	...
	return appendMachineSetValues(values, infraStatus, cluster.Shoot.Annotations), nil
}
...
func appendMachineSetValues(values map[string]interface{}, infraStatus *apisazure.InfrastructureStatus, annotations map[string]string) map[string]interface{} {
	values["vmType"] = "standard"
	if azureapihelper.IsVmoRequired(infraStatus, annotations) {
		values["vmType"] = "vmss"
		return values
	}
...
func getCSIControllerChartValues(
	...
	if azureapihelper.IsVmoRequired(infraStatus, cluster.Shoot.Annotations) {
		values["vmType"] = "vmss"
	} else {
		values["vmType"] = "standard"
	}
...
func getControlPlaneShootChartValues(
	...
	return map[string]interface{}{
		...
		azure.AllowEgressName: map[string]interface{}{
			"enabled": infraStatus.Networks.OutboundAccessType == apisazure.OutboundAccessTypeLoadBalancer,
		},
