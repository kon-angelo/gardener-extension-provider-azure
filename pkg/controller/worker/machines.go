...
func (w *workerDelegate) generateMachineConfig(ctx context.Context) error {
	...
	for _, pool := range w.worker.Spec.Pools {
		...
		for zoneIndex, zone := range pool.Zones {
			...
			switch infrastructureStatus.Networks.TopologyType {
			case azureapi.TopologyZonalSingleSubnet:
				nodesSubnet, err := azureapihelper.FindSubnetByPurpose(infrastructureStatus.Networks.Subnets, azureapi.PurposeNodes)
				if err != nil {
					return err
				}
			...
			case azureapi.TopologyZonal:
				zoneInt, err := strconv.Atoi(zone)
				if err != nil {
					return err
				}
				index, nodesSubnet, err := azureapihelper.FindSubnetByPurposeAndZone(infrastructureStatus.Networks.Subnets, azureapi.PurposeNodes, int32(zoneInt))
				if err != nil {
					return err
				}
				subnetName = nodesSubnet.Name
				if index == 0 {
					...
					workerPoolHash, err = w.generateWorkerPoolHash(pool, infrastructureStatus, vmoDependency, &subnetName)
					if err != nil {
						return err
					}
				}
