...
func (t TerraformAdapter) Zones() []zoneTf {
	...
	for _, subnet := range rawSubnets.([]map[string]interface{}) {
		...
		ipAddrRaw, ok := natRaw["ipAddresses"]
		var userManagedIPs []userManagedIP
		if ok {
			ipAddrs := ipAddrRaw.([]map[string]interface{})
			for _, addr := range ipAddrs {
				ipName := addr["name"].(string)
				ipRgroup := addr["resourceGroup"].(string)
				userManagedIPs = append(userManagedIPs, userManagedIP{ipName, ipRgroup, *zone})
			}
		}

		res = append(res, zoneTf{
			rawNetNumber:                 rawNetNumber,
			enabled:                      natRaw["enabled"].(bool),
			idleConnectionTimeoutMinutes: idleConnectionTimeoutMinutes,
			migrated:                     isMigrated,
			clusterName:                  t.ClusterName(),
			zone:                         zone,
			cidr:                         cidr,
			serviceEndpoints:             serviceEndpoints,
			ipAddresses:                  userManagedIPs,
		})
	}
	return res
}
...
func (nat zoneTf) UserManagedIP() []userManagedIP {
	return nat.ipAddresses
}
