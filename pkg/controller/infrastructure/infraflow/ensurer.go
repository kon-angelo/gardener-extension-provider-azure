...
func (f *FlowContext) EnsureVirtualNetwork(ctx context.Context) error {
	...
	f.whiteboard.Set(KindVirtualNetwork.String(), *vnet.ID)
	return nil
}
...
func (f *FlowContext) ensureManagedVirtualNetwork(ctx context.Context) (*armnetwork.VirtualNetwork, error) {
	...
	if vnet != nil {
		if location := pointer.StringDeref(vnet.Location, ""); location != f.adapter.Region() {
			log.Error(NewTerminalSpecMismatch(vnetCfg.AzureResourceMetadata, "location", f.adapter.Region(), location), "error occurred but reconciliation will continue.")
		}
	}
...
func (f *FlowContext) EnsureAvailabilitySet(ctx context.Context) error {
	...
	f.whiteboard.Set(KindAvailabilitySet.String(), *avset.ID)
	return nil
}
...
func (f *FlowContext) ensureAvailabilitySet(ctx context.Context, log logr.Logger, avsetCfg AvailabilitySetConfig) (*armcompute.AvailabilitySet, error) {
	...
	if avset != nil {
		if location := pointer.StringDeref(avset.Location, ""); location != f.adapter.Region() {
			log.Error(NewTerminalSpecMismatch(avsetCfg.AzureResourceMetadata, "location", f.adapter.Region(), location), "error occurred but reconciliation will continue")
		}
		return avset, nil
	}
...
func (f *FlowContext) EnsureRouteTable(ctx context.Context) error {
	...
	f.whiteboard.Set(KindRouteTable.String(), *rt.ID)
	return nil
}
...
func (f *FlowContext) ensureRouteTable(ctx context.Context) (*armnetwork.RouteTable, error) {
	...
	if rt != nil {
		if location := pointer.StringDeref(rt.Location, ""); location != f.adapter.Region() {
			log.Error(NewTerminalSpecMismatch(rtCfg.AzureResourceMetadata, "location", f.adapter.Region(), location), "error occurred but reconciliation will continue")
		}
		return rt, nil
	}
...
func (f *FlowContext) ensureSecurityGroup(ctx context.Context) (*armnetwork.SecurityGroup, error) {
	...
	if sg != nil {
		if location := pointer.StringDeref(sg.Location, ""); location != f.adapter.Region() {
			log.Error(NewTerminalSpecMismatch(sgCfg.AzureResourceMetadata, "location", f.adapter.Region(), location), "error occurred but reconciliation will continue")
		}
		return sg, nil
	}
...
func (f *FlowContext) ensureUserPublicIp(ctx context.Context, c client.PublicIP, ipCfg PublicIPConfig) error {
	...
	f.whiteboard.GetChild(KindPublicIP.String()).GetChild(ipCfg.ResourceGroup).Set(ipCfg.Name, *userIP.ID)
	return nil
}
...
func (f *FlowContext) ensurePublicIps(ctx context.Context) error {
	var (
		log         = f.LogFromContext(ctx)
		toDelete    = map[string]*armnetwork.PublicIPAddress{}
	...
	for name, pipCfg := range managedIPConfigs {
		toReconcile[name] = pipCfg.ToProvider(nameToCurrentIps[name])
	}
...
func (f *FlowContext) ensureNatGateways(ctx context.Context) error {
	...
	var (
		log         = f.LogFromContext(ctx)
		joinError   error
		toDelete    = map[string]*armnetwork.NatGateway{}
	...
	nats, err := c.List(ctx, f.adapter.ResourceGroupName())
	if err != nil {
		return err
	}
...
func (f *FlowContext) GetInfrastructureState() (*runtime.RawExtension, error) {
	state := &v1alpha1.InfrastructureState{
		...
		Data:     f.whiteboard.ExportAsFlatMap(),
	}

	for _, v := range f.inventory.ToList() {
